...
df.micro = df.micro %>% filter(function. != 'ratio_transform')
...
df.micro = df.micro %>% group_by(program, size, precision, function., mode, JIT) %>% filter(time == min(time)) %>% select(!rep) %>% select(!logprob)

df.micro[df.micro$precision == 32, ]$program = 'torchtree32'


df.physher2 = df.micro %>% filter(function. == 'ratio_transform_jacobian2') %>% mutate(program =
                                                                                         'physher2') %>% mutate(function. = 'ratio_transform_jacobian')
...
df.micro.jax.1 = df.micro.jax %>% filter(grepl('1', mode)) %>% mutate(mode =
                                                                        str_sub(mode, 1, nchar(mode) - 1))
...
### Gradient evalutation wihtout phylojax
```{r}
micro.grad = df.micro %>% filter(mode == 'gradient') %>%
  filter(program != 'physher2') %>%
...
### Gradient evalutation log scale
```{r}
...
### Log likelihood evalutation log scale
```{r}
...
```{r}
dd = df.micro %>%  filter(program %in% c('physher', 'physher2', 'bitorch')) %>%
  filter(function. != 'coalescent') %>%
  filter(program != 'physher' | function. != 'ratio_transform_jacobian')
...
ggplot(micro.join, aes(
  ...
  geom_point() +
  facet_wrap( ~ function., scales = "free") +
  labs(title = "Ratio of gradient and likelihood execution time")
```