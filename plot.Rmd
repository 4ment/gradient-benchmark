...
#colors = palette.colors(NULL, "Okabe-Ito")
colors = hue_pal()(length(programs))
progColors <- setNames(colors, programs)
...
shift_legend <- function(p) {
  ...
  if (!"gtable" %in% class(p)) {
    if ("ggplot" %in% class(p)) {
      ...
      message(
        "This is neither a ggplot object nor a grob generated from ggplotGrob. Returning original plot."
      )
      return(p)
    }
  } else {
  ...
  empty.facet.panels <-
    sapply(facet.panels, function(i)
...
df.trace[df.trace$program == 'bitorch',]$program = 'torchtree/BITO'

ggplot(df.trace, aes(x = size,
                     y = max_peak_vmem,
                     color = program)) +
  geom_line() +
  geom_point() +
...
df.macro.raw[df.macro.raw$program == 'bitorch', ]$program = 'torchtree/BITO'
...
```{r}
df.macro = df.macro.raw %>%
  filter(program %in%  c('bitorch', 'torchtree/BITO', 'physher')) %>%
  group_by(program, size) %>% summarize(time = min(time)) %>% ungroup()
...
df.micro = df.micro %>%
  mutate(program = ifelse(function. == 'treelikelihoodAnalytic',
                          'jax-analytic',
                          program))  %>%
...
p <- ggplot(micro.like, aes(
  ...
  geom_line() +
  geom_point() +
  facet_wrap( ~ function., scales = "free", nrow = 2,strip.position = "top") +
  xlab("Number of sequences") +
  ylab("Time in seconds for evaluation") +
  scale_color_manual(values = progColors) +
...
micro.jax.grad = df.micro.jit %>% filter(mode == 'gradient') %>%
  filter(program == 'phylojax')

ggplot(micro.jax.grad, aes(
  x = size,
  y = time,
  color = JIT,
  linetype = JIT
...
```{r micro-grad-log}
dd = df.micro %>% filter(mode == 'gradient') %>% filter(program != 'phylojax1')
